...
module.exports = function registerLookups(app, pool) {
  ...
  app.get('/faculties', async (req, res) => {
    try {
      const { rows } = await pool.query('SELECT id, name FROM public.faculties ORDER BY name');
    ...
    try {
      ...
      if (facultyId) {
        const { rows } = await pool.query(
          'SELECT id, name, faculty_id FROM public.departments WHERE faculty_id = $1 ORDER BY name',
          [facultyId]
        );
        return res.json(rows);
      } else {
        const { rows } = await pool.query('SELECT id, name, faculty_id FROM public.departments ORDER BY name');
        return res.json(rows);
      }
    } catch (err) {
